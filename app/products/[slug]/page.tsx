...
interface ProductFull extends Omit<Product, "createdAt" | "updatedAt" | "author"> {
  ...
  author?: { name: string; image?: string; title?: string | null };
    ...
    <main className="min-h-screen bg-white">
      <ProductSchema 
        ...
        author={product.author ? {
          ...
          avatar: product.author.image
        } : null}
      />
      <BreadcrumbSchema
        items={[
          { name: "Home", url: process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000" },
          { name: "Products", url: `${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/products` },
          { name: product.name, url: `${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/products/${product.slug}` },
        ]}
      />
          ...
          <div className="mt-6 sm:mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
            ...
            <div className="lg:col-span-7 xl:col-span-7">
              <div className="relative aspect-[16/10] sm:aspect-[16/9] rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden border border-white shadow-2xl shadow-blue-900/10 bg-white">
                ...
                <div className="absolute top-4 left-4 sm:top-6 sm:left-6 flex flex-col gap-2">
                  <span className="px-4 py-1.5 bg-white/90 backdrop-blur-md rounded-full text-[10px] sm:text-xs font-bold text-blue-800 uppercase tracking-widest shadow-sm border border-blue-100/50">
                    {product.subcategory.categoryType === "nutra" ? "Supplement" : "Premium Gear"}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3 sm:gap-4 mt-6 sm:mt-8">
                <div className="bg-white p-3 sm:p-5 rounded-2xl sm:rounded-3xl border border-gray-100 flex flex-col items-center text-center group hover:border-blue-200 transition-colors shadow-sm">
                  ...
                  <span className="text-[10px] sm:text-xs font-bold text-slate-900 uppercase tracking-tight">Pure Quality</span>
                </div>
                <div className="bg-white p-3 sm:p-5 rounded-2xl sm:rounded-3xl border border-gray-100 flex flex-col items-center text-center group hover:border-blue-200 transition-colors shadow-sm">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-blue-50 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                    <Truck className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600" />
                  </div>
                  <span className="text-[10px] sm:text-xs font-bold text-slate-900 uppercase tracking-tight">Fast Shipping</span>
                </div>
                <div className="bg-white p-3 sm:p-5 rounded-2xl sm:rounded-3xl border border-gray-100 flex flex-col items-center text-center group hover:border-blue-200 transition-colors shadow-sm">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-blue-50 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                    <RotateCcw className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600" />
                  </div>
                  <span className="text-[10px] sm:text-xs font-bold text-slate-900 uppercase tracking-tight">Easy Returns</span>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-5 xl:col-span-5 flex flex-col">
              <div className="flex items-center gap-2 mb-3 sm:mb-4">
                ...
                <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Verified Product</span>
              </div>
                  ...
                  <div className="mb-10 p-5 sm:p-6 bg-white rounded-[2rem] border border-slate-100 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5 sm:gap-6">
                      <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-2xl overflow-hidden shadow-md border-4 border-white flex-shrink-0 ring-1 ring-slate-100">
                        <Image
                          src={product.author.image || "/avatar-placeholder.png"}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1.5">
                          <span className="text-lg font-black text-slate-900 tracking-tight">{product.author.name}</span>
                          <div className="flex items-center gap-1 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100">
                            <CheckCircle2 className="w-3 h-3 text-blue-600" />
                            <span className="text-[10px] font-bold text-blue-700 uppercase tracking-widest">Verified Expert</span>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 gap-x-6">
                          <div className="flex items-center gap-2 text-slate-500">
                            ...
                            <span className="text-xs font-medium">{product.author.title || "Health Researcher"}</span>
                          </div>
                          <div className="flex items-center gap-2 text-slate-400">
                            <Activity className="w-3.5 h-3.5" />
                            <span className="text-xs">Updated: {new Date(product.updatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                          </div>
                        </div>
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-4 border-t border-slate-50 pt-3">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[10px] text-slate-400 font-medium uppercase tracking-tighter">Rating</span>
                          </div>
                          <div className="h-3 w-px bg-slate-200" />
                          <div className="flex items-center gap-1.5">
                            <ShieldCheck className="w-3.5 h-3.5 text-blue-500" />
                            <span className="text-xs font-bold text-slate-700">12+ yrs</span>
                            <span className="text-[10px] text-slate-400 font-medium uppercase tracking-tighter">Experience</span>
                          </div>
                          <div className="h-3 w-px bg-slate-200" />
                          <div className="flex items-center gap-1.5">
                            <ShoppingCart className="w-3.5 h-3.5 text-emerald-500" />
                            <span className="text-xs font-bold text-slate-700">500+</span>
                            <span className="text-[10px] text-slate-400 font-medium uppercase tracking-tighter">Products Reviewed</span>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  {product.ingredients && product.ingredients.length > 0 && (
                    <div className="mt-8 bg-slate-900 rounded-[2rem] p-6 text-white shadow-xl shadow-slate-900/20 overflow-hidden relative group">
                      ...
                      <div className="flex items-center gap-3 mb-6 relative z-10">
                        ...
                        <div>
                          <h3 className="text-lg font-bold leading-tight">Active Ingredients</h3>
                          <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold mt-0.5">Potency & Purity</p>
                        </div>
                      </div>

                      <div className="space-y-4 relative z-10">
                        {product.ingredients.map((ing) => (
                          <div key={ing.id} className="flex items-center gap-4 group/item">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-bold text-white group-hover/item:text-blue-400 transition-colors">{ing.name}</p>
                              <div className="flex items-center gap-1.5 mt-1">
                                <Activity className="w-3 h-3 text-blue-500" />
                                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">Bioavailable Form</span>
                              </div>
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="mt-8 pt-6 border-t border-slate-800 relative z-10">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-xs font-bold text-slate-400">Scientific Trust</span>
