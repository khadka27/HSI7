  ...
  return (
    <Link href={`/product/${product.slug}`} className="group block float-in">
      <div className="surface-shell rounded-3xl hover:shadow-xl hover:shadow-emerald-950/10 transition-all duration-300 hover:-translate-y-1 overflow-hidden">
        <div className="relative aspect-square overflow-hidden bg-emerald-50/60">
          <img
            src={product.image}
            alt={product.name}
        ...
        <div className="p-5">
          <h3 className="text-base md:text-lg font-semibold text-slate-900 line-clamp-2 leading-snug group-hover:text-emerald-700 transition-colors">
            {product.name}
          </h3>
